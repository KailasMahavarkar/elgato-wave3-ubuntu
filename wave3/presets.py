...
EQUALISER = [
    Preset("Default", "High-pass only, otherwise flat", bands=(
        ...
        (eq.BELL, 155.0, 0.0, 1.0),
        (eq.BELL, 650.0, 0.0, 1.2),
        (eq.BELL, 2650.0, 0.0, 1.4),
        (eq.BELL, 7700.0, 0.0, 1.0),
        (eq.HISHELF, 12000.0, 0.0, 0.7),
    )),
    Preset("Flat", "Every band off, no shaping at all", bands=(
        (eq.OFF, 80.0, 0.0, 0.7),
        (eq.OFF, 155.0, 0.0, 1.0),
        (eq.OFF, 650.0, 0.0, 1.2),
        (eq.OFF, 2650.0, 0.0, 1.4),
        (eq.OFF, 7700.0, 0.0, 1.0),
        (eq.OFF, 12000.0, 0.0, 0.7),
    )),
    Preset("Broadcast", "Radio voice: tight lows, clear presence", bands=(
        (eq.HIPASS, 90.0, 0.0, 0.7),
        (eq.BELL, 180.0, -2.0, 1.0),
        (eq.BELL, 500.0, -3.0, 1.2),
        (eq.BELL, 3000.0, 3.0, 1.0),
        (eq.BELL, 7000.0, 1.5, 1.0),
        (eq.HISHELF, 12000.0, 2.0, 0.7),
    )),
    Preset("Warm", "Fuller and closer, less top end", bands=(
        (eq.HIPASS, 70.0, 0.0, 0.7),
        (eq.BELL, 200.0, 3.0, 1.0),
        (eq.BELL, 600.0, -1.5, 1.2),
        (eq.BELL, 2000.0, -1.0, 1.4),
        (eq.BELL, 6000.0, -1.0, 1.0),
        (eq.HISHELF, 12000.0, -1.5, 0.7),
    )),
    Preset("Bright", "Cuts mud, opens the top", bands=(
        (eq.HIPASS, 100.0, 0.0, 0.7),
        (eq.BELL, 220.0, -2.5, 1.0),
        (eq.BELL, 600.0, -4.0, 1.4),
        (eq.BELL, 3500.0, 3.0, 1.0),
        (eq.BELL, 8000.0, 2.0, 1.0),
        (eq.HISHELF, 12000.0, 2.5, 0.7),
    )),
    Preset("De-boom", "For close mic work with heavy proximity", bands=(
        (eq.HIPASS, 120.0, 0.0, 0.8),
        (eq.BELL, 250.0, -5.0, 1.2),
        (eq.BELL, 700.0, -2.0, 1.2),
        (eq.BELL, 2500.0, 2.0, 1.2),
        (eq.BELL, 7000.0, 1.0, 1.0),
        (eq.HISHELF, 12000.0, 1.5, 0.7),
    )),
]
